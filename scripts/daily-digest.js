...
async function getRepoActivity(repo, since) {
  const [commits, prs, issues, releases] = await Promise.all([
    octokit.repos
      .listCommits({ owner: ORG, repo, since })
      .catch(() => ({ data: [] })),
    octokit.pulls.list({
      owner: ORG,
      repo,
    ...
    octokit.repos
      .listReleases({ owner: ORG, repo, per_page: 10 })
      .catch(() => ({ data: [] })),
  ]);
  ...
  const recentIssues = issues.data.filter(
    (i) => !i.pull_request && new Date(i.updated_at) > sinceDate,
  );
      ...
      messages: [
        {
          role: "user",
          content:
            `You are writing a daily engineering digest for a startup founder/tech lead. ` +
...
(async () => {
  ...
  const activity = await Promise.all(
    repos.map((r) => getRepoActivity(r, since)),
  );
  const raw = buildRawSummary(activity);
  const digest = await generateTechnicalDigest(raw);
  await sendEmail(
    `Daily Engineering Digest — ${new Date().toDateString()}`,
    digest,
  );
